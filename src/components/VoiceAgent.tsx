...
const STATUS_LABELS: Record<string, string> = {
  idle: "Tap to speak with a support agent",
...
export function VoiceAgent() {
  const { status, transcript, isSpeaking, error, startCall, endCall } =
    useVapi();
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-xl mx-auto">
      ...
      <div className="relative flex items-center justify-center mb-8">
        ...
        {isActive && (
          <>
            ...
            <div
              className="absolute w-32 h-32 rounded-full bg-rp-teal-500/10 pulse-ring"
        ...
        <button
          ...
          className={`relative z-10 w-24 h-24 rounded-full flex items-center justify-center transition-all duration-200 focus:outline-none focus-visible:ring-4 focus-visible:ring-rp-blue-300 ${
            isActive
      ...
      {/* Status label */}
      <p className="text-sm font-medium text-rp-muted mb-6 tracking-wide">
        {isActive && (
          <span className="inline-flex items-center gap-2">
            <span
              className="w-2 h-2 rounded-full bg-rp-teal-500 dot-blink inline-block"
              aria-hidden="true"
            />
            {STATUS_LABELS[status]}
          </span>
        )}
        {!isActive && STATUS_LABELS[status]}
      </p>

      {/* Audio wave — shown while agent is speaking */}
      ...
      {isIdle && !error && (
        <p className="text-xs text-rp-subtle mt-1">
          Microphone access required to start a call
        </p>
      )}
